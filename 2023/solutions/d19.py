    ...
    def possible_values(self, values: set[int], negated: bool = False):
        ...
        if self.op == '<':
            for rating in values:
                if (not negated and rating < self.val) or (
                    negated and rating >= self.val
                ):
                    new.add(rating)
        elif self.op == '>':
            for rating in values:
    ...
    for line in workflow_str.splitlines():
        ...
        for wf in prog.split(','):
            if len(wf) > 1 and wf[1] in '<>':
                part, op, val, result = re.match(
                    r'(x|m|a|s)(<|>)([0-9]+):([a-z]+|A|R)', wf
                ).groups()
...
def solve2(ipt: PartWorkflows):
    return get_combos(
        ipt.workflows,
        'in',
        {part: frozenset(i for i in range(1, 4001)) for part in 'xmas'},
    )
...
def get_combos(
    workflows: dict[str, list[Workflow]], name: str, accepted: dict[str, frozenset[int]]
):
    if name == 'A':
    ...
    for test in workflows[name]:
        ...
        for ctest in completed_tests:
            new_accepted[ctest.part] = ctest.possible_values(
                new_accepted[ctest.part], negated=True
            )
