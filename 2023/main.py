...
def main():
    ...
    parser.add_argument(
        ...
        '--day',
        dest='day',
        help='Runs day <d>. If -f is not specified, '
...
def print_table(outputs: list[Result]):
    ...
    for p1, p2, out in zip(part1_lines, part2_lines, outputs):
        if len(p1) < len(p2):
            ...
            print(
                '│ {:>{day}} │ {:<{part1}} │ {:<{part2}} │ {:>{time}.3f} │'.format(
                    day_num_file(out.day),
                    p1[0],
                    p2[len(p2) // 2],
                    out.time,
                    day=day_width,
    ...
    if len(outputs) > 1:
        ...
        print(
            f'│ {"Total Time":^{day_width + width1 + width2 + 6}} │ {sum([out.time for out in outputs]):>{time_width}.3f} │'
        )
        print(f'╰{"─" * (day_width + width1 + width2 + 8)}┴{"─" * (time_width + 2)}╯')
    else:
        print(
            f'╰{"─" * (day_width + 2)}┴{"─" * (width1 + 2)}┴{"─" * (width2 + 2)}┴{"─" * (time_width + 2)}╯'
        )


def generate_new_file(day_number):
    path = os.path.join('solutions', f'd{day_number:0>2}.py')
